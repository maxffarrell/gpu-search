...
- Chrome production-build checks cover exact/alias/no-match behavior, invalid JSON, literal HTML-like labels, no horizontal overflow at 390px, dark mode, and no script errors. Zero requests were observed after assets loaded during those query/edit interactions. See `bench/reports/demo.json`.
...
- The size report separately compresses all emitted resources with Node's zlib Brotli quality11/window22 and gzip level9. The lexical library meets the 8KiB target. There are no model assets or source maps in the demo transfer.
