/** Isolated research runtime; excluded from the demo and default package import graphs. */
import { features } from './features.js';
import { bigramFeatures, BIGRAM_FEATURE_VERSION } from './features-bigram.js';
...
export async function loadBigramModel(input: unknown, payload: ArrayBuffer): Promise<Model> {
  ...
  requireAsset(manifest.featureVersion === 'gpu-search-features-v1' || manifest.featureVersion === BIGRAM_FEATURE_VERSION, 'Unsupported featureVersion');
  const useBigrams = manifest.featureVersion === BIGRAM_FEATURE_VERSION;
  ...
  function encode(text: string): Float32Array {
    ...
    const extracted = useBigrams ? bigramFeatures(text) : { ...features(text), bigramIds: [] as number[] };
    const pooled = new Float32Array(DIMENSION), bigrams = extracted.bigramIds;
    const families: [number[], number, number][] = [
      [extracted.wordIds, 0, bigrams.length ? 0.375 : 0.5],
      [extracted.charIds, 1, 0.5],
      [bigrams, 0, 0.125],
    ];
    for (const [ids, family, weight] of families) {
      ...
      for (let d = 0; d < DIMENSION; d++) pooled[d] = Math.fround(pooled[d]! + Math.fround(Math.fround(mean[d]! / ids.length) * weight));
    }
