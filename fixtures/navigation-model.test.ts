...
const fixture = JSON.parse(readFileSync(new URL('navigation-model.json', import.meta.url), 'utf8')) as {
  modelSha256: string;
  candidates: { id: string; label: string; aliases?: string[]; context?: string }[];
  cases: { text: string; vector: number[]; scores: number[] }[];
};
test('selected navigation int8 model matches independent NumPy embeddings and composed menu scores', async () => {
